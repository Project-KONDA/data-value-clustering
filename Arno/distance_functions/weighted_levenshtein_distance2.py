...
def get_cost(cost_map, c1, c2):
    ...
    if lower(c1) == lower(c2):
        return cost_map[()]
    k1 = get_my_kind(cost_map, c1)
    k2 = get_my_kind(cost_map, c2)
    return cost_map[(k1, k2)]


def levenshtein_distance2(cost_map, s1, s2):
    d = {}
    l1 = len(s1)
    l2 = len(s2)
    d[(-1, -1)] = 0
...
if __name__ == "__main__":
    ...
    #print(match(" ", " "))
    #      != None)
    print(get_my_kind(map, ""))
    print(get_my_kind(map, "a"))
    print(get_my_kind(map, "A"))
    print(get_my_kind(map, "1"))
    print(get_my_kind(map, " "))
    print(get_my_kind(map, "-"))
