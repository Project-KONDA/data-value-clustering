...
def min_samples_config(no_values, answers):
    name = "min_samples"
    explanation = "Larger values will yield less clusters."
    min_min_samples = 3
    ...
    suggestion_min = max(no_values/50 * noise_factor, min_min_samples)  # TODO: experiment
    suggestion_max = min(no_values/5 * noise_factor, max_min_samples)  # TODO: experiment
    suggestion_value = (suggestion_max - suggestion_min) / 2  # TODO: experiment
...
def eps_config(distance_matrix, no_values):
    ...
    # min_eps = min(distances_to_k_nearest)
    # max_eps = max_value_distance or max(distances_to_k_nearest)
