...
def menu_help_abstraction(master, restricted):
    ...
    text = [
        ...
        "As the screenshot below shows, the abstraction of the first 100 data values often does not provide a sufficient overview of the whole abstraction. In this example, only two simple clusters are shown.",
        "",
        ["Confirm & Discard"],
...
def menu_help_blob_input(master, restricted):
    ...
    text = [
        ...
        "",
        ["Size"],
        "The size of each blob represents the weight of substituting characters of the represented group by each other. Some blobs cannot be scaled. They represent groups of characters that were replaced by the same character during the abstraction, thus for example integers and letter sequences in the screenshot below. Their size is fixed to the minimal possible size and they are slightly less saturated. Which blobs are fixed depends on your configuration of the abstraction.",
        # interpretation
...
def menu_help_result(master, restricted):
    ...
    text = [
        ...
        "A clustering is compact if there is a high similarity within clusters and it is separate if there is a low similarity between clusters. In general, a good clustering should have high compactness and separation. If the colours are very mixed, this may hint at the low meaningfulness of the clustering (cf. first evaluation question)." + ("" if restricted else "The plot may also assist you in answering the second evaluation question.") + " Make sure to also check the entire clustering provided as an Excel file before answering the questions.",
        "",
        ["Clustering"],
