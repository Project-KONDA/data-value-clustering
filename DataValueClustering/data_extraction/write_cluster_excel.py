...
def cluster_to_excel(path, clusters, noise, clusters_compressed, noise_compressed, comp_to_normal_map, cluster_sizes,
    ...
    style_caption = workbook.add_format({'bold': True, 'font_color': 'red', 'left': 2, 'font_size': 15, 'bg_color': '#DDDDDD'})
    style_sum = workbook.add_format({'bold': True, 'font_color': 'blue', 'left': 2, 'bottom': 1, 'bg_color': '#DDDDDD'})
    style_sum_right = workbook.add_format({'right': 2, 'bottom': 1, 'bg_color': '#DDDDDD'})
    style_value = workbook.add_format({'left': 2})
    style_grey = workbook.add_format({'right': 2, 'bg_color': '#DDDDDD'})
    ...
    write_list_to_sheet(sheet1, 3, 2, noise_count, style_occurrence, True)
    ...
    for i, v in enumerate(cluster_indices_sorted):
        ...
        sheet1.write_number(2, i * 2 + 3, cluster_counts_sorted[v], style_sum)
    ...
    if not comp_to_normal_map is None:
        ...
        write_list_to_sheet(sheet2, 4, 2, noise_count, style_occurrence, True)
        ...
        for i, v in enumerate(cluster_compressed_indices_sorted):
            ...
            sheet2.write_number(2, i * 2 + 3, cluster_counts_sorted[v], style_sum)
