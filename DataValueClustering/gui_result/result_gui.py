...
def show_mds_scatter_plot_integrated(root, values_compressed, distance_matrix, clusters_compressed, savepath=None, show=True,
    ...
    a.scatter(out[:, 0], out[:, 1], color=value_colors)
    # plt.scatter(out[:, 0], out[:, 1], color=value_colors)

    a.set_title("MDS Scatter Plot", fontsize=12)
    ...
    def quit_figure(event):
        if event.key == 'enter':
            a.close(event.canvas.figure)

    # a.gcf().canvas.mpl_connect('key_press_event', quit_figure)
